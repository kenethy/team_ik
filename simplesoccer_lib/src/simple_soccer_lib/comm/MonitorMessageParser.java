...
public class MonitorMessageParser {
	...
	private void parse_showinfo_t() {
		...
		this.matchPerception.setState(EMatchState.valueOf((int)pmode));
		...
		for (int i = 0; i < 11; i++) {
			...
			parse_pos_t_Player(this.matchPerception.getTeamName(EFieldSide.LEFT), playersA.get(i));
		}
		
		//time 1
		for (int i = 0; i < 11; i++) {
			playersB.add(new PlayerPerception());
			parse_pos_t_Player(this.matchPerception.getTeamName(EFieldSide.RIGHT), playersB.get(i));
		}
		...
		this.fieldPerception.setTeamPlayers(EFieldSide.LEFT, playersA);
		this.fieldPerception.setTeamPlayers(EFieldSide.RIGHT, playersB);
	}
	...
	*/
	private void parse_team_t(boolean isTeamA) {
		...
		if(isTeamA){
			this.matchPerception.setTeamName(EFieldSide.LEFT, name);
			this.matchPerception.setTeamScore(EFieldSide.LEFT, score);
		}else{
			this.matchPerception.setTeamName(EFieldSide.RIGHT, name);
			this.matchPerception.setTeamScore(EFieldSide.RIGHT, score);
		}
	}
	...
	*/
	...
	private void parse_pos_t_Player(String team, PlayerPerception playerPerception) {
		...
		playerPerception.setSide(EFieldSide.valueOf(side));
		playerPerception.setState(EPlayerState.valueOf(state));
